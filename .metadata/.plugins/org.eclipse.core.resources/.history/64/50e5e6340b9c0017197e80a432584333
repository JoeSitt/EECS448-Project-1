...
public class NewTime implements Comparable<NewTime> {
	...
	public int compareTo(NewTime nt) {
		if (this == null || nt == null) {
			System.out.println("ERROR: null NewTime comparison");
			return 0;
		}
		
		if (this.milHours > nt.milHours) {
			return 1;
		} else if (this.milHours < nt.milHours) {
			return -1;
		} else {
			if (!this.onHour && nt.onHour) {
				return 1;
			} else if (this.onHour && !nt.onHour) {
				return -1;
			} else {
				return 0;
			}
