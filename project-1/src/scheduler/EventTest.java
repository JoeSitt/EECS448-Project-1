...
public class EventTest {
	...
	public void testParseDateString() {
		String dateString = "02/05/2014";
		
		Date testDate = Event.parseDateString(dateString);
		Event testEvent = new Event(testDate, "");
		
		assertEquals(testEvent.getDateString(), dateString);
	}
	...
	/**
	 * Test that event comparison is done correctly
	 */
	@Test
	public void testCompareTo() {
		Event lowEvent = new Event(new Date(), "A");
		Event highEvent = new Event(new Date(), "Z");
		
		assertEquals(lowEvent.compareTo(highEvent), -25);
		assertEquals(highEvent.compareTo(lowEvent), 25);
		
		Event equalEvent1 = new Event(new Date(), "J");
		Event equalEvent2 = new Event(new Date(), "J");
		
		assertEquals(equalEvent1.compareTo(equalEvent1), 0);
		assertEquals(equalEvent2.compareTo(equalEvent2), 0);
		assertEquals(equalEvent1.compareTo(equalEvent2), 0);
		assertEquals(equalEvent2.compareTo(equalEvent1), 0);
	}
